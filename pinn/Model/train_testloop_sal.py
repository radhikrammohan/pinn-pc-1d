...
def training_loop(epochs, model, \
                  loss_fn_data, optimizer, adaptiveloss,\
    ...
    for epoch in range(epochs):
        ...
        for (batch, batch_pde, batch_init, batch_left, batch_right) in \
            ...
            # Backpropagation
            loss = adaptiveloss(phy_loss, init_loss, bc_loss)  # Use self-adaptive loss
        ...
        for (batch, batch_pde, batch_init, batch_left, batch_right) in zip_longest(test_dataloader, \
            ...
            bc_loss_t = (bc_loss_left_t + bc_loss_right_t)
            ...
            # Use self-adaptive loss
            loss_t = adaptiveloss(phy_loss_t, init_loss_t, bc_loss_t)
        ...
        if epoch % 10 == 0:
            ...
            print(f"--"*50)
            print(f"| Data-loss {data_loss_b/len(train_dataloader):.4e},| pde-loss {phy_loss_acc /len(train_dataloader):.4e},| initc-loss {init_loss/len(train_dataloader):.4e},|bc_loss {bc_loss/len(train_dataloader):.4e}|") 
            print(f"--"*50)
