...
def training_loop(epochs, model, \
    ...
    for epoch in range(epochs):
        ...
        for (batch, batch_pde, batch_init, batch_left, batch_right) in \
            ...
            phy_loss = pde_loss(model, inputs_pde[:, 0].unsqueeze(1), inputs_pde[:, 1].unsqueeze(1), T_st, T_lt,epoch)  # PDE loss
        ...
        for (batch, batch_pde, batch_init, batch_left, batch_right) in zip_longest(test_dataloader, \
            ...
            phy_loss_t = pde_loss(model, inputs_pde[:, 0].unsqueeze(1), inputs_pde[:, 1].unsqueeze(1), T_st, T_lt,epoch)  # PDE loss
            
            w0, w1, w2, w3 = 1,1,1,1
            # loss_t = data_loss_t  # Total loss
