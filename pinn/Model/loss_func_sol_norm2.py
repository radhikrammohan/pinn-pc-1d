...
def cp_ramp(temp,v1,v2,T_L,T_S):        # Function to calculate specific heat capacity in Mushy Zone
    ...
    cp_max = torch.maximum(v1, v2)  # Ensure cp_m does not exceed the maximum of v1 and v2
    cp_m_sc = cp_m/ cp_max  # Scale cp_m to be between 0 and 1
    return cp_m_sc
...
def pde_loss(model,x,t,T_S,T_L):
    ...
    def Ste(u_pred):
        ...
        delta_T = T_L - T_S
        Ste = (cp_ramp(u_pred,cp_l_t,cp_s_t,T_L,T_S)*delta_T)/ 1
        return Ste
...
def boundary_loss(model,x,t,t_su,t_in):
    ...
    t_surr_c = torch.full_like(u_pred, t_su)
...
def ic_loss(model,x,t,temp_ini):
    ...
    temp_i = temp_ini + 50 * torch.sin(torch.pi * x)
    # # u_del = u_pred - temp_init
    temp_i = torch.full_like(u_pred,temp_ini)
