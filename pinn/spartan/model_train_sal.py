...
from Model.train_testloop_sal import *
...
# %%
# rand_smpl = RandomSampler(inp_dataset, replacement=False,num_samples=1000)  # random sampler for training/simulation data
# rand_smpl_pde = RandomSampler(inp_pde_dataset, replacement=False, num_samples=1000) # random sampler for pde residuals-training
# rand_smpl_ic = RandomSampler(inp_ic_dataset, replacement=False, num_samples=1000)  # random sampler for ic residuals-training
# rand_smpl_bcl = RandomSampler(inp_bcl_dataset, replacement=False, num_samples=1000) # random sampler for bc left residuals-training
# rand_smpl_bcr = RandomSampler(inp_bcr_dataset, replacement=False, num_samples=1000) # random sampler for bc right residuals-training

# rand_smpl_test = RandomSampler(inp_dataset_test, replacement=False, num_samples=1000 )  # random sampler for testing/simulation data
# rand_smpl_pde_test = RandomSampler(inp_pde_dataset_test,replacement=False, num_samples=1000)  # random sampler for pde residuals
# rand_smpl_ic_test = RandomSampler(inp_ic_dataset_test,replacement=False, num_samples=1000)  # random sampler for ic residuals
# rand_smpl_bcl_test = RandomSampler(inp_bcl_dataset_test,replacement=False, num_samples=1000) # random sampler for bc left residuals
# rand_smpl_bcr_test = RandomSampler(inp_bcr_dataset_test,replacement=False, num_samples=1000) # random sampler for bc right residuals
batch_size = 153
train_loader = DataLoader(inp_dataset, batch_size=batch_size) # training data loader
pde_loader = DataLoader(inp_pde_dataset, batch_size=batch_size) # pde residual data loader training
ic_loader = DataLoader(inp_ic_dataset, batch_size=batch_size) # ic residual data loader training
bcl_loader = DataLoader(inp_bcl_dataset, batch_size=batch_size) # bc left residual data loader training
bcr_loader = DataLoader(inp_bcr_dataset, batch_size=batch_size) # bc right residual data loader training


test_loader = DataLoader(inp_dataset_test, batch_size=batch_size) # testing data loader
pde_loader_test = DataLoader(inp_pde_dataset_test, batch_size=batch_size)
ic_loader_test = DataLoader(inp_ic_dataset_test, batch_size=batch_size)
bcl_loader_test = DataLoader(inp_bcl_dataset_test, batch_size=batch_size)
bcr_loader_test = DataLoader(inp_bcr_dataset_test, batch_size=batch_size)
...
from Model.selfadaploss import SelfAdaptiveLoss

model = PINN(input_size, hidden_size, output_size,hidden_layers).to(device)
adaptiveloss = SelfAdaptiveLoss()
...
optimizer_1 = torch.optim.Adam(
    list(model.parameters()) + list(adaptiveloss.parameters()), lr=learning_rate)  # Adam optimizer for training
...
loss_train,loss_test,best_model = training_loop(epochs_1, model, loss_fn_data, \
                  optimizer_1,adaptiveloss,train_loader,pde_loader, ic_loader,\
