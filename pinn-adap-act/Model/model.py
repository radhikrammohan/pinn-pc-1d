...
class AdaptiveTanh(nn.Module):
    def __init__(self, alpha=0.1):
        ...
        self.alpha = nn.Parameter(torch.tensor(alpha))  # Learnable parameter for scaling
        # self.n = nn.Parameter(torch.tensor(5.0))  # Learnable parameter for scaling
    def forward(self, x):
        return torch.tanh(10 * self.alpha * x)  # Apply the adaptive tanh activation function
...
class PINN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, hidden_layers):  # Constructor initializes the network
        ...
        layers.append(AdaptiveTanh())  # Apply the activation function (Tanh)
        ...
        for _ in range(hidden_layers):  # Loop for creating multiple hidden layers
            ...
            layers.append(AdaptiveTanh())  # Add an activation function (Tanh)
